...
int main(int argc, char ** argv) {
  int k,p,me,left,right,count = 1, tag = 1, nit = 200, N = 8;
  ...
  if (me == 0) u_arr[0] = alpha; if (me == p-1) u_arr[z - 1] = beta;
  for (k = 1; k <= nit; k ++) {
    //if (me % 2 == 0) {
      ...
      if (me < p-1) MPI_Recv (&u_arr[z-1], count, MPI_FLOAT, right, tag, MPI_COMM_WORLD, &status);
      if (me > 0)   MPI_Recv (&u_arr[0], count, MPI_FLOAT, left , tag, MPI_COMM_WORLD, &status);
      //}
      //else {
  ...
  for (int m = 0; m <= me; m ++) {
    if (me == m) {
      ...
      for(int j = 0; j < z; j++) { printf ("U[%d]=%f; ",j,u_arr[j]); }
           }
    printf ("\n");
